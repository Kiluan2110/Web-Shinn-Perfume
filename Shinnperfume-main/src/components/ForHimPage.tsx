...
// Force reload: Fixed For Him duplicate images with 10 UNIQUE Unsplash images - 2024-12-18T10:35:00
...
export function ForHimPage({ onBack }: ForHimPageProps) {
  ...
  const searchResults = searchQuery.trim() === '' 
    ? [] 
    : himPerfumes.filter((perfume) =>
        perfume.name.toLowerCase().includes(searchQuery.toLowerCase())
      );

  const handleSelectPerfume = (index: number) => {
    const actualIndex = himPerfumes.findIndex(p => p.id === searchResults[index].id);
  ...
  const handlePrevious = () => {
    setDirection('up');
    setCurrentIndex((prev) => (prev > 0 ? prev - 1 : himPerfumes.length - 1));
  };
  ...
  if (!currentPerfume || himPerfumes.length === 0) {
    return (
      <div className="h-screen bg-black flex items-center justify-center">
        <div className="text-center">
            ...
            <div className="px-4 md:px-12 py-4 md:py-8">
              ...
              <div className="md:hidden space-y-3">
                ...
                <div className="flex items-center justify-between">
                  <motion.button
                    onClick={onBack}
                    className="text-white/80 hover:text-blue-400 transition-colors tracking-[0.2em] text-xs uppercase"
                ...
                >
                  ...
                  <input
                    ...
                    className="w-full bg-black/30 backdrop-blur-sm text-white pl-10 pr-4 py-2 border border-white/30 rounded-full focus:outline-none focus:border-blue-400 focus:bg-black/50 transition-all duration-300 placeholder:text-white/40"
              ...
              <div className="hidden md:flex items-center justify-between">
                <motion.button
                  onClick={onBack}
                  className="text-white/80 hover:text-blue-400 transition-colors tracking-[0.3em] text-sm uppercase"
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                  style={{ fontFamily: 'Cinzel, serif' }}
                >
                ...
                >
                  ...
                  <input
                    ...
                    onChange={(e) => handleSearch(e.target.value)}
                    placeholder="Search perfumes..."
                    className="bg-black/30 backdrop-blur-sm text-white pl-10 pr-4 py-2 border border-white/30 rounded-full focus:outline-none focus:border-blue-400 focus:bg-black/50 transition-all duration-300 placeholder:text-white/40 w-56"
                    style={{ fontFamily: 'Cinzel, serif', fontSize: '0.75rem', letterSpacing: '0.1em' }}
                  />
                        ...
                        <div className="max-h-[500px] overflow-y-auto">
                          {searchResults.map((perfume, index) => (
                            <motion.button
                              key={perfume.id}
                              onClick={() => handleSelectPerfume(index)}
                              className="w-full flex items-center gap-4 p-4 hover:bg-blue-500/20 transition-all duration-300 border-b border-white/10 last:border-b-0"
                              whileHover={{ x: 5 }}
                            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-8">
                ...
                <div className="border-t border-white/30 pt-3 md:pt-4">
                  ...
                  <div className="space-y-1 md:space-y-2">
                    <div>
                      <p 
                        className="text-blue-300 text-[0.65rem] md:text-xs mb-0.5 md:mb-1 tracking-wider uppercase"
                        style={{ fontFamily: 'Cinzel, serif' }}
                      >
                        Top
                      </p>
